...
staggering\footnote{see the sections on {\em National Importance} and
  {\em Pathways to Impact.}} and there are many successful approaches to
...
implementations of the same specification) and extensional reasoning
(proving that programs with the same behaviour are the same). These
limitations are manifestations of an even %more fundamental
deeper  problem
unresolved for 40 years, namely the lack of a satisfactory
...
equality. And then one must address the question of when such
proofs are equal! This leads to an intricate, higher-dimensional
structure which we have so far not understood.


%This leads to the question of when such
%proofs are equal, resulting in an intricate, higher-dimensional
%structure which we have so far not understood.
...
certainty is undermined by the risk of making mistakes in proofs. The
advent of computers raised the possibility once more of achieving in
practice the promise of mathematical certainty. This potential is now
coming to fruition, \eg systems such as Coq have been able to formally
verify both large mathematical theorems such as the Four Colour
Theorem, and large software systems such as the CompCert C compiler.
However, these systems are not {\em extensional}: behaviourally
indistinguishable objects cannot be proven to be the same. 
This fundamental problem significantly hinders reusability and modularity.
As extensionality is a consequence of HoTT, our system
...
ambitious initial phase we will take an axiomatic approach to HoTT, \eg
adding univalence and HITs as postulates. Later, we will be  more ambitious by i) formally verifying key
...
formalisation of HoTT within itself. Working within a HoTT-based
system will produce more reusable and modular
proofs --- important as our formalisation needs to adapt to
developments in other work packages. If possible, we will simplify our
formalisation by using inductive-recursive HITs developed in WP3.  
%Overall, WP5 ensures the required level of trust in our system and
Clearly, WP5 impacts
the formal verification community. %as the first example of formal
%verification in a {\em HoTT-based} environment 
WP5 also establishes
...
depend on WP5. {\em Deliverable: Formally verified proofs of
  ...
  written in it. Collaborator: Matthieu Sozeau.  }
...
{\bf WP8: Impact II --- HoTT and Units of Measure.}  We will validate
...
% reusability and modularity. %when
...
%In addition to advancing our ability to
